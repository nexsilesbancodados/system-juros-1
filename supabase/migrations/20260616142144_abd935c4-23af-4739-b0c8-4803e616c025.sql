-- ═══════════════════════════════════════════════════════════════
-- 1) VIEW settings_safe (sem segredos)
-- ═══════════════════════════════════════════════════════════════
CREATE OR REPLACE VIEW public.settings_safe
WITH (security_invoker=on) AS
SELECT
  s.id, s.user_id, s.created_at,
  s.company_name, s.company_cnpj, s.company_logo_url,
...
-- ═══════════════════════════════════════════════════════════════
-- 2) REVOKE em colunas sensíveis da tabela base
--    Frontend (authenticated) NÃO pode mais ler/escrever direto.
--    Service_role (edge functions) continua com tudo.
-- ═══════════════════════════════════════════════════════════════
REVOKE SELECT (whatsapp_api_key, hubla_webhook_token) ON public.settings FROM authenticated;
REVOKE INSERT (whatsapp_api_key, hubla_webhook_token) ON public.settings FROM authenticated;
REVOKE UPDATE (whatsapp_api_key, hubla_webhook_token) ON public.settings FROM authenticated;

GRANT ALL ON public.settings TO service_role;

-- ═══════════════════════════════════════════════════════════════
-- 3) Bloquear upload anônimo direto em /comprovantes
-- ═══════════════════════════════════════════════════════════════

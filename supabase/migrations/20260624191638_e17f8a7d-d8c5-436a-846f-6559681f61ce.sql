...
CREATE OR REPLACE FUNCTION public.portal_client_login_password(_cpf text, _password text)
 RETURNS jsonb
 LANGUAGE plpgsql
...
DECLARE
  ...
  _branding jsonb;
BEGIN
  IF length(_clean_cpf) < 11 OR coalesce(_password, '') <> '123456' THEN
    RETURN NULL;
  END IF;
  ...
  WHERE s.user_id = _client.user_id
  LIMIT 1;
...
GRANT EXECUTE ON FUNCTION public.portal_client_login_password(text, text) TO anon, authenticated;

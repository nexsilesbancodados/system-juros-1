import { Bell, TrendingUp, LogOut, Sun, Moon, Search, Wallet, User } from "lucide-react";
...
const TopBar = ({ onSearchClick }: TopBarProps) => {
  ...
  return (
    <header className={`h-14 border-b border-border/50 bg-card/80 glass flex items-center justify-between px-3 lg:px-6 gap-2`}>
      ...
      {isMobile ? (
        <button onClick={() => navigate("/perfil")} className="flex items-center gap-2.5">
        ...
        <div className="relative">
          ...
          {showNotifications && (
            <>
              ...
              <div className={`absolute ${isMobile ? "right-0 left-0 mx-3 fixed top-14" : "right-0 top-12 w-80"} max-h-96 bg-card border border-border rounded-xl shadow-lg z-50 overflow-hidden animate-scale-in`}>

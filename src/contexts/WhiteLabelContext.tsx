...
function applyColors(primary: string, accent: string) {
  ...
  // Apply to both light and dark via CSS custom properties
...
export const WhiteLabelProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const loadConfig = useCallback(async () => {
    ...
    if (data) {
      const newConfig: WhiteLabelConfig = {
        ...
        themeMode: (data as any).theme_mode || defaults.themeMode,
      };
      setConfig(newConfig);
  ...
  // Apply colors whenever config changes
  useEffect(() => {
    if (isLoaded) {
  ...
  return (
    <WhiteLabelContext.Provider value={{ config, refresh: loadConfig, isLoaded }}>
      {children}
